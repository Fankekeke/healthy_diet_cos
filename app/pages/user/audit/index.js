...
Page({
  ...
  isLogin() {
    wx.getStorage({
      key: 'userInfo',
      success: (res) => {
        this.getAuditInfo(res.data.userId)
      },
      fail: res => {
        wx.showToast({
          title: '请先进行登录',
    ...
    wx.getStorage({
      ...
      success: (res) => {
        if (this.data.tag == '' || this.data.content == '' || this.data.fileList === 0) {
          ...
          let data = {tag: this.data.tag, introduction: this.data.content, userId: res.data.userId, images: images.length !== 0 ? images.join(',') : null}
