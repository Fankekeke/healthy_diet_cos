...
Page({
    ...
    queryCollectPost() {
        wx.getStorage({
          key: 'userInfo',
          success: (res) => {
           http.get('queryFocusUser', { focusUserId: this.data.shopInfo.id, userId: res.data.userId }).then((r) => {
      ...
      collectPost() {
        wx.getStorage({
          key: 'userInfo',
          success: (res) => {
           http.get('focusUser', { focusUserId: this.data.shopInfo.id, userId: res.data.userId, type: 1 }).then((r) => {
            this.queryCollectPost()
          })
          }
        })
      ...
      collectPostElse() {
        wx.getStorage({
          key: 'userInfo',
          success: (res) => {
           http.get('focusUser', { focusUserId: this.data.shopInfo.id, userId: res.data.userId, type: 2 }).then((r) => {
            this.queryCollectPost()
          })
          }
        })
      ...
      chatUser() {
        wx.getStorage({
            ...
            success: (res) => {
                wx.navigateTo({
                    url: '/pages/user/detail/index?takeUser=' + this.data.shopInfo.id + '&sendUser=' + res.data.userId + '&otherName=' + this.data.shopInfo.name + ''
                });
            },
            fail: res => {
                wx.showToast({
